...
class AddRForm extends React.Component {
    ...
    checkPlacement() {
        let checkTypeErrorFlag = checkType(this.state.type);
        if (checkTypeErrorFlag > 0) {
            this.setState({flag: checkTypeErrorFlag});
            return
        }

        let checkFloorErrorFlag = checkFloor(this.state.floor);
        if (checkFloorErrorFlag > 0) {
            this.setState({flag: checkFloorErrorFlag});
            return
        }

        let checkWCountErrorFlag = checkWCount(this.state.winCount);
        if (checkWCountErrorFlag > 0) {
            this.setState({flag: checkWCountErrorFlag});
            return
        }

        let checkAreaErrorFlag = checkArea(this.state.area);
        if (checkAreaErrorFlag > 0) {
            this.setState({flag: checkAreaErrorFlag});
            return
        }
